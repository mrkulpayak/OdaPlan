...
export function RadialRotateMenu({
  ...
  // RadialRotateMenu is rendered without any parent rotation transform.
  // Screen angle == SVG local angle, so no compensation needed.
  ...
  useEffect(() => {
    ...
    const initTip = polarToXY(cx, cy, r3 + 15 / zoom, initFaceAngle);
    ...
    const onMove = (e: PointerEvent) => {
      ...
      // No parent transform — screen angle == local angle directly.
      if (indicatorRef.current) {
        const tip = polarToXY(cx, cy, r3 + 15 / zoom, snappedFaceAngle);
    ...
    const onUp = () => {
      // Apply the final indicator angle (covers zone 4 where we deferred onAngleChange)
      const finalRotation = ((lastSnappedFaceAngleRef.current - fo) % 360 + 360) % 360;
      onAngleChange(finalRotation);
      onConfirm();
    };
  ...
  const sw = 0.8 / zoom;
  const fs = Math.max(7, r0 * 0.24);
  const arrowLen  = 15 / zoom;
  const arrowHalf = 7 / zoom;

  return (
    <g style={{ pointerEvents: 'none' }}>
      <defs>
        <marker
          id="radial-indicator-arrow"
          markerWidth={arrowLen} markerHeight={arrowHalf * 2}
          refX={arrowLen} refY={arrowHalf}
          orient="auto"
      ...
      {/* ── Outer disc — 65% opaque ── */}
      <circle cx={cx} cy={cy} r={r3} fill="rgba(255,255,255,0.65)" stroke="rgba(0,0,0,1)" strokeWidth={sw} />
      ...
      {TICKS_5.map((a) => {
        ...
        return <line key={`t5-${a}`} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y}
          stroke="rgba(0,0,0,1)" strokeWidth={0.5 / zoom} strokeLinecap="round" />;
      })}

      {/* ── Ring divider r2 ── */}
      <circle cx={cx} cy={cy} r={r2} fill="none" stroke="rgba(0,0,0,1)" strokeWidth={sw} />
      ...
      {TICKS_45.map((a) => {
        ...
        return <line key={`t45-${a}`} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y}
          stroke="rgba(0,0,0,1)" strokeWidth={0.9 / zoom} strokeLinecap="round" />;
      })}

      {/* ── Ring divider r1 ── */}
      <circle cx={cx} cy={cy} r={r1} fill="none" stroke="rgba(0,0,0,1)" strokeWidth={sw} />
      ...
      {TICKS_90.map((a) => {
        ...
        return <line key={`t90-${a}`} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y}
          stroke="rgba(0,0,0,1)" strokeWidth={1.4 / zoom} strokeLinecap="round" />;
      })}

      {/* ── Inner dead-zone disc — 65% opaque ── */}
      <circle cx={cx} cy={cy} r={r0} fill="rgba(255,255,255,0.65)" stroke="rgba(0,0,0,1)" strokeWidth={sw} />
      ...
      <line
        ...
        stroke="var(--color-primary)" strokeWidth={2.8 / zoom} strokeLinecap="round"
        markerEnd="url(#radial-indicator-arrow)"
      />

      {/* ── Center circle + delta readout — topmost layer ── */}
      ...
      <text x={cx} y={cy + fs * 0.38} textAnchor="middle"
        style={{ fontSize: fs, fontFamily: 'var(--font-mono)', fill: '#fff', fontWeight: 700, pointerEvents: 'none' }}>
